...
type Step = "auth" | "company" | "confirm";
type AuthMethod = "email" | "google" | "facebook" | "microsoft" | null;

const SIGNUP_ERROR_MESSAGES: Record<string, string> = {
    oauth_signup_failed: "No se pudo crear la cuenta. El nombre o email ya está en uso.",
    session_save_failed: "Error al iniciar el proceso. Intentá de nuevo.",
...
export default function Signup() {
    ...
    const canProceedAuth = form.ownerName.length >= 2 && /^[^@]+@[^@]+\.[^@]+$/.test(form.email) && validatePassword(form.password) && form.password === form.confirmPassword;
    const canProceedCompany = form.companyName.length >= 2 && form.slug.length >= 3 && slugCheck.data?.available && acceptedTerms;

    const isOAuth = authMethod !== null && authMethod !== "email";
    ...
    useEffect(() => {
        ...
        if (error) {
            const msg = SIGNUP_ERROR_MESSAGES[error] || `Error: ${error}`;
            toast.error(msg);
    ...
    const providerLabel = (m: AuthMethod) => {
    ...
    // For OAuth: only 2 steps (auth choice → company + OAuth). For email: 3 steps.
    const allSteps: Step[] = isOAuth ? ["auth", "company"] : ["auth", "company", "confirm"];
        ...
        <div className="min-h-screen relative overflow-hidden bg-background">
            ...
            <div className="min-h-screen flex items-center justify-center p-4">
                <div className="w-full max-w-lg">
                        ...
                        <Card className="glass-card border-border/50 shadow-2xl shadow-purple-500/10">
                            ...
                            <CardContent className="space-y-4">
                                            ...
                                            {enabledProviders.includes('google') && (
                                                <Button
                                                    ...
                                                    className={`w-full h-11 bg-card/50 hover:bg-card border-border/50 transition-all duration-300 hover:border-blue-500/30 hover:shadow-md transform hover:scale-[1.02] active:scale-[0.98] group ${
                                                        authMethod === 'google' ? 'ring-2 ring-purple-500 border-purple-500/50' : ''
                                                    }`}
                                            ...
                                            {enabledProviders.includes('facebook') && (
                                                <Button
                                                    ...
                                                    className={`w-full h-11 bg-card/50 hover:bg-card border-border/50 transition-all duration-300 hover:border-blue-600/30 hover:shadow-md transform hover:scale-[1.02] active:scale-[0.98] group ${
                                                        authMethod === 'facebook' ? 'ring-2 ring-purple-500 border-purple-500/50' : ''
                                                    }`}
                                            ...
                                            {enabledProviders.includes('microsoft') && (
                                                <Button
                                                    ...
                                                    className={`w-full h-11 bg-card/50 hover:bg-card border-border/50 transition-all duration-300 hover:border-blue-700/30 hover:shadow-md transform hover:scale-[1.02] active:scale-[0.98] group ${
                                                        authMethod === 'microsoft' ? 'ring-2 ring-purple-500 border-purple-500/50' : ''
                                                    }`}
                                ...
                                <Button
                                    ...
                                    disabled={authMethod ? false : !canProceedAuth}
                                    onClick={() => {
                    ...
                    {step === "company" && (
                        <Card className="glass-card border-border/50 shadow-2xl shadow-purple-500/10">
                            <CardHeader className="text-center pb-4">
                                ...
                                <CardDescription>
                                    {isOAuth
                                        ? `Completa los datos y se abrirá ${providerLabel(authMethod)} para autenticarte`
                            ...
                            <CardContent className="space-y-4">
                                ...
                                {/* Terms acceptance — required for all paths */}
                                ...
                                <div className="flex gap-3">
                                    <Button
                                        ...
                                        onClick={() => { setStep('auth'); setAuthMethod(null); setAcceptedTerms(false); }}
                                    >
                                    ...
                                    {isOAuth ? (
                                        <Button
                                            ...
                                            onClick={() => handleOAuthSignup(authMethod as 'google' | 'facebook' | 'microsoft')}
                                        >
                                            Crear con {providerLabel(authMethod)}
                                            <ArrowRight className="ml-2 h-4 w-4" />
                                        </Button>
                                    ) : (
                                        <Button
                    ...
                    {/* Step 3: Confirm (email registration only) */}
                    {step === "confirm" && (
                        <Card className="glass-card border-border/50 shadow-2xl shadow-purple-500/10">
                            <CardHeader className="text-center pb-4">
                                <CardTitle className="text-xl flex items-center justify-center gap-2">
